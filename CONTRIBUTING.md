...
参与开发前须事先取得书面许可，详见 [LICENSE](LICENSE)。取得授权后，可改进文件解析、识别规则、人工复核体验和测试覆盖。提交问题或 PR 时请使用合成资料，不要上传真实合同、证件、恢复包、密码、工作文件或日志中的个人信息。
