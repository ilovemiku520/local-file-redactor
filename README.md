...
本地文件脱敏是一款面向中文文档的桌面浏览器工具，将**本地大模型、OCR、规则识别和人工逐页复核**结合起来，识别并遮除姓名、电话、邮箱、证件号、银行账号、地址和自定义敏感词。支持 Word、Excel、PDF、文本、CSV 和常见图片，并可通过密码加密的恢复包还原完整原件。
...
原生 Word/Excel 导出已从“新建文档”改为“修改原 OOXML 内容”。跨字体片段的敏感信息也会逐字符移除，不再把整个段落或工作表重排。旧任务需要重新上传识别，才能使用保留格式导出。详见[排版验证记录](docs/LAYOUT_PRESERVATION.md)。
...
| 原格式与样式保留 | 在 DOCX/XLSX 原结构中替换文字，保留字体、字号、颜色、段落、合并表格、页眉页脚、行高列宽与打印设置 |
...
> 自动识别不能保证零漏检。导出前必须逐页检查，尤其是手写、印章、人脸、二维码、低清晰度文字及复杂排版。DOCX/XLSX 原格式导出保留样式；内容预览和 Office 转 PDF 仍为简化排版。遮罩字宽或阅读软件差异可能影响自动换行，不能承诺像素一致。反脱敏恢复完整原件，不合并后续编辑。
...
| `.docx` | `.docx` / `.pdf` | 原 DOCX 字体/段落/分节/表格/页眉页脚/脚注/文本框位置保留；可选 PDF 为简化版 |
| `.xlsx` | `.xlsx` / `.pdf` | 原 XLSX 样式、合并、行高列宽、冻结窗格、数字格式和打印设置保留；公式转缓存值 |
| `.pdf` | `.pdf` | 300 DPI 渲染、OCR/文字层定位、像素遮挡、重建无原文字层的 PDF |
| `.png` / `.jpg` / `.jpeg` / `.webp` / `.bmp` | 上述任一图片类型 | 单帧图片、像素遮挡、重新编码；默认 PNG |

旧版 DOC/XLS、PPT/PPTX、宏文件、多帧图片暂不支持。Office 图表、SmartArt、数学公式/嵌入对象、Excel 非图片绘图/旧式页眉图片、外部数据或没有缓存值的公式，会要求先转为静态文件。隐藏内容不会直接带入成品。详见[限制与数据处理](docs/ARCHITECTURE.md)。

## 界面预览

### 候选复核与导出

左侧预览中的红框表示待遮除区域，右侧列出候选及来源。复核与恢复截图来自 2.0 版合成 Word 样例，正文和插图分别复核；顶部主页截图已更新至 2.1。
...
下图与下载成品保留自 **2.0 版真实合成样例试验**，用于展示识别、图片遮除和恢复流程；2.1 的原格式保留验证见[新版本测试记录](docs/LAYOUT_PRESERVATION.md)。右图直接取自已导出 DOCX 的嵌入图片，未对结果图做美化或额外遮挡；OCR 当前按识别行生成框，因此可能遮除整行。
...
| 验证项目 | 已观察结果 | 范围 |
|---|---|---|
| 2.1 默认回归（2026-09-14） | 41 通过、12 跳过 | 含新增 8 项基础排版测试；默认跳过模型/OCR 集成用例 |
| 2.1 部署环境验证（2026-09-14） | 12 通过 | 8 项重复的基础排版测试、3 项真实 API 流程、1 项真实 OCR 排版与恢复；合计 45 个不同用例 |
| 2.0 历史回归（2026-09-09） | 41 个不同测试用例通过 | 含真实本地模型/OCR、文件恢复、上传保护、PDF 旋转与历史清理；未全部重跑 |
...
这些结果是功能回归与小样本试运行，**不是检出率或零泄露证明**。没有完成 120 份文件、300 页、3000 个标注实体的系统基准，未声称 99% 准确率。更多记录见[2.1 排版验证](docs/LAYOUT_PRESERVATION.md)、[2.0 试验说明](docs/VALIDATION.md)和[2.1 机器可读回归摘要](docs/benchmarks/layout-checks.json)。
...
backend/app/
  ...
  formats.py       文本解析、格式分发和内容复核预览
  office.py        OOXML 原结构脱敏、文字样式和布局保留
...
- DOCX/XLSX 原格式输出保留布局属性和字体样式；遮罩字宽、字体缺失和阅读软件差异可能影响换行。内容预览与 Office 转 PDF 仍为简化版。需要固定页面外观时，请先在 Office 中另存为 PDF 后脱敏。
- Excel 条件格式和数据验证会清除，静态样式保留；不保留宏、批注、修订历史或外部链接。PDF 成品保留页面视觉外观，但不保留可编辑文字层。
...
取得书面授权后，可通过合成案例改进解析、规则、复核交互和测试。请阅读[贡献指南](CONTRIBUTING.md)。项目原创内容保留所有权利，使用须经事先书面许可，详见 [LICENSE](LICENSE)，第三方模型和依赖仍遵循各自[上游许可](THIRD_PARTY_NOTICES.md)。
